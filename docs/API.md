...
| `lockfile` | `discoverLockfileGraphs`' result: merged `packages` (each a `DiscoveredPackage` — `name`, `version`, optional `license`, `devDeclared`, `scope`, and optional `integrity`/`resolved` carried verbatim from the lockfile entry, absent when it had none — see "Lockfile integrity and resolved" below for what happens when two sightings disagree), `rootDependencies`, `edges`, the `files` read, `diagnostics`. |
...
**`DiscoveredPackage.integrity` / `.resolved`** ride verbatim in the lockfile's own spelling — npm's `integrity` is SRI (`sha512-<base64>`), never converted to hex or otherwise normalized here; a consumer that wants a CycloneDX `hashes[]` entry (algorithm name + hex digest) does that conversion itself. pnpm-lock.yaml carries the equivalent data as `resolution.integrity`/`resolution.tarball` on the `packages` map's VALUE (the key is only `name@version`). Both are absent, never guessed, when the lockfile entry carried neither. `mergeDiscovered` merges an identical `integrity` across two sightings of the same `name@version` silently; two *different, both-present* values drop the field to absent on the merge and add `NPM_INTEGRITY_CONFLICT: …` to `lockfile.diagnostics` naming the package and both hashes, rather than picking one — a mismatched hash for one resolved name@version is a supply-chain signal. Once a conflict is recorded the field stays absent for the rest of the fold, even if a later sighting happens to agree with an earlier value, and **regardless of where the disagreeing sightings sit** — two paths of one lockfile, or two lockfiles, reach the same merged record. That permanence lives in the `MergeFold` a caller threads through (`{ diagnostics?, integrityConflicts }`, `createMergeFold()`), keyed by `name@version` rather than by record identity, so it survives every point a `DiscoveredPackage` is copied and applies whichever side of a merge the conflicted record arrives on; `discoverLockfileGraphs` threads one fold through every lockfile it reads, and `parsePackageLockJsonGraph(path, integrityConflicts?)` takes that set as an optional second argument. `resolved` has no conflict handling and keeps the first value seen (legitimate mirrors can differ).
...
import {
  ...
  parsePackageLockJsonGraph, parsePnpmLockYamlGraph, parsePackageLockJson, parsePnpmLockYaml,
  mergeDiscovered, createMergeFold, // fold two sightings of one name@version; a fold's diagnostics sink + conflict memory
