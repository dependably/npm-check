...
```json
{
  ...
  "lockfile": {
    ...
    "packages": [
      { "name": "leaf", "version": "2.0.0", "devDeclared": false, "integrity": "sha512-leaf", "resolved": "https://registry.npmjs.org/leaf/-/leaf-2.0.0.tgz" },
      { "name": "lib-a", "version": "1.0.0", "devDeclared": false, "integrity": "sha512-liba", "resolved": "https://registry.npmjs.org/lib-a/-/lib-a-1.0.0.tgz" }
    ],
...
`lockfile` is the dependency graph every `package-lock.json` and `pnpm-lock.yaml` under the tree records (outside `node_modules`; several lockfiles merge into one graph): the resolved `packages` (each with the `license` the lockfile carries, a tri-state `devDeclared` — `true` reachable only through devDependencies, `false` some path reaches it without a dev edge, absent when the lockfile says nothing — `scope: "optional"` only when npm asserts exclusivity, and `integrity`/`resolved` — see "Lockfile integrity and resolved" below), the `rootDependencies` the project's own manifests depend on directly, the `edges` among the closure (hoisting-accurate for npm; peer-suffix-stripped for pnpm), the `files` that were read, and `diagnostics` (`NPM_LOCKFILE_UNPARSEABLE: …`, `NO_LOCKFILE`, or `NPM_INTEGRITY_CONFLICT: …` — see "Diagnostic codes" below).

### Lockfile integrity and `resolved`

Every `package-lock.json` and `pnpm-lock.yaml` entry that carries an artifact hash and/or a resolved download URL passes them through on `DiscoveredPackage` as `integrity` and `resolved` — optional, absent when the lockfile entry carried neither. **Both ride VERBATIM, in the lockfile's own spelling.** npm records `integrity` as an SRI string (`sha512-<base64>`); this is never converted to hex, never normalized, and never guessed when absent — the facts layer reports what the file says, and a transformed value is no longer quotable evidence. The consumer (e.g. a CycloneDX `hashes[]` entry, which wants an algorithm name plus a hex digest) does the conversion. `resolved` is likewise the lockfile's own URL, unmodified.

pnpm-lock.yaml carries `resolution.integrity` and (for a non-registry source: a tarball URL, a git dependency — pnpm can otherwise derive a registry download URL itself) `resolution.tarball` on the `packages` map's VALUE — the map's key is only `name@version`.

**A conflicting hash for one `name@version` across paths or lockfiles is never silently resolved by picking one.** `mergeDiscovered`'s fold rule: two sightings with the *same* `integrity` merge normally; two sightings with *different, both-present* `integrity` values drop the field to absent on the merged package and emit `NPM_INTEGRITY_CONFLICT: …` naming the package and both hashes — a mismatched hash for one resolved name@version is a supply-chain signal, not a merge nuisance, and picking either value silently would hide it. Once two sightings disagree the field stays absent for the rest of the fold; a later sighting cannot reinstate it even if it happens to agree with one of the earlier values, and it makes no difference whether the disagreeing pair sat in one lockfile or in two — the conflict is remembered per `name@version` for the whole run, not per record. `resolved` has no such conflict handling (multiple mirrors legitimately serve a package at different URLs) and keeps the first value seen.

### Diagnostic codes

Every diagnostic string in `workspace.diagnostics` and `lockfile.diagnostics` starts with a stable `CODE:` prefix, so a consumer can classify a diagnostic by matching the prefix rather than pattern-matching prose. Severity is deliberately **not** part of the facts document — grading a diagnostic (note vs. warning vs. error) is the consumer's call; this only identifies the condition.

| Code | Where | Meaning |
| --- | --- | --- |
| `NPM_MANIFEST_UNPARSEABLE` | `workspace.diagnostics` | A `package.json` under the tree failed to parse as JSON. It is skipped — no `firstPartyNames`/`depScopes` contribution from it — and the rest of the workspace discovery continues. |
| `NPM_TSCONFIG_UNPARSEABLE` | `workspace.diagnostics` | A `tsconfig*.json`/`jsconfig*.json` failed to parse. The aliases it would have declared are ignored; discovery continues. |
| `OUTPUT_DIR_SCANNED` | `workspace.diagnostics` | A directory whose name suggests generated output (`build`, `coverage`, `dist`, `out`, `vendor`) was scanned as first-party source because nothing in the tree's `.gitignore` excluded it. A **note**, not a warning — the scan read *more* of the tree, not less. |
| `NPM_LOCKFILE_UNPARSEABLE` | `lockfile.diagnostics` | A `package-lock.json`/`pnpm-lock.yaml` under the tree failed to parse. That file contributes nothing to the merged graph; every other lockfile is still read. |
| `NO_LOCKFILE` | `lockfile.diagnostics` | No `package-lock.json` or `pnpm-lock.yaml` was found anywhere under the tree; no npm packages could be enumerated. |
| `NPM_INTEGRITY_CONFLICT` | `lockfile.diagnostics` | Two sightings of the same `name@version` (different paths, or different lockfiles) carried different `integrity` hashes. The merged package's `integrity` drops to absent rather than picking one — a supply-chain signal, not a merge nuisance. |
