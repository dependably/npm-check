...
### Fixed
- **Two `docs/CLI.md` envelope samples named a version this build could not emit.** The facts sample printed `"toolVersion": "1.10.1"` beside `"schemaVersion": "1.2"` — an impossible pair, since 1.10.1 shipped facts schema `1.1` and `1.2` first exists in this release; a reader gating on the tool version would have drawn the wrong conclusion from the canonical example. The findings sample was merely stale at `1.7.0`. Both now name this build, and `tests/unit/docs-envelope-consistency.test.js` makes the impossible pair unrepresentable: every ```json envelope in `docs/` and `README.md` must carry this package's version and the schemaVersion its own `documentType` selects (facts documents carry the discriminator, the findings envelope does not). Verified to fail against the old literal.

### Added
- **`DiscoveredPackage` now carries lockfile `integrity` and `resolved`, for both npm and pnpm** — CISA's 2026 SBOM Minimum Elements makes Component Hash Value/Algorithm required fields, and the facts layer previously dropped both at construction even though every lockfile it parses records at least the hash. npm's `package-lock.json` per-entry `integrity`/`resolved` and pnpm-lock.yaml's per-package `resolution.integrity`/`resolution.tarball` (read from the `packages` map's VALUE — only the map's KEY was read before) now ride straight onto `DiscoveredPackage`, in the lockfile's own verbatim spelling: npm's SRI string (`sha512-<base64>`) is never converted to hex or otherwise normalized, and both fields are absent — never guessed, never defaulted — when the entry carried neither. A CycloneDX `hashes[]` entry (algorithm name + hex digest) is the consumer's conversion to make, not this layer's.
  - **`mergeDiscovered` gained a fold rule for `integrity`/`resolved`.** A matching or one-sided `integrity` across two sightings of the same name@version (two paths in one lockfile, or two lockfiles in one workspace) merges silently; two sightings that DISAGREE drop the field to absent on the merged record — a mismatched hash for one resolved package is a supply-chain signal, not a merge nuisance a fold should paper over — and add a coded `NPM_INTEGRITY_CONFLICT: …` diagnostic naming the package and both hashes. Once a conflict is recorded it is permanent for the rest of the fold, per `name@version`: a third sighting cannot silently reinstate the field even if it happens to agree with one of the earlier values, whether the disagreeing pair sat in one lockfile or in two, and whichever side of the later merge the conflicted record arrives on. `resolved` merges by simple first-value-wins with no conflict check, since legitimate mirrors can serve one package at different URLs.
  - `mergeDiscovered`'s signature gained an optional third `fold?: MergeFold` parameter — `{ diagnostics?: string[], integrityConflicts: Set<string> }`, the state one fold carries: where to route the diagnostic, and which `name@version` keys have already had `integrity` dropped. `createMergeFold()` builds an empty one. The two fields travel together deliberately: the conflict set is the memory behind the diagnostic, so a caller cannot report a conflict the fold is unable to remember. Omitting `fold` is still supported for a standalone pair of records — the conflict resolves to absent, it is just unreported and not remembered past the call. `parsePackageLockJsonGraph` gained a matching optional second argument (`integrityConflicts?: Set<string>`) so `discoverLockfileGraphs` can thread ONE fold through every lockfile it reads.
    - **The conflict memory is keyed by `name@version`, never by record identity.** An earlier draft used a `WeakSet` of record objects, whose stated premise ("every caller folds into the same record object") was false: `discoverLockfileGraphs` folds a fresh `{ ...pkg }` copy at the per-lockfile boundary, and a spread drops the mark, so a conflict found between two paths of one lockfile was forgotten the moment a second, agreeing lockfile was merged in — the run reported `NPM_INTEGRITY_CONFLICT: …; dropped` and published a hash in the same breath. The reverse (an already-conflicted record arriving as the merge SOURCE) failed the same way. A value-level fact that is repeatedly copied cannot be keyed on identity, so the key is the value.
  - **`LockfileGraph.diagnostics` is a new REQUIRED property on an exported interface** — `parsePackageLockJsonGraph`/`parsePnpmLockYamlGraph` now always return it (empty when nothing conflicted), and `discoverLockfileGraphs` folds every graph's entries up into `LockfileDiscovery.diagnostics`. Reading the field is additive, but a consumer that CONSTRUCTS a `LockfileGraph` (a test stub, a fake reader) fails to typecheck until it adds `diagnostics: []`.
  - **`FACTS_SCHEMA_VERSION` bumped `1.1` → `1.2`.** Both new `DiscoveredPackage` fields are additive and optional, so a `1.1`-aware consumer still parses `lockfile.packages` unchanged.
  - Documented in `docs/API.md` / `docs/CLI.md` ("Lockfile integrity and resolved"; `NPM_INTEGRITY_CONFLICT` added to the "Diagnostic codes" table). checker-npm#37.

### Changed
- **The GitHub mirror's `release` job no longer publishes to public npmjs — it publishes to no registry at all.** `.github/workflows/ci.yml` ran `npm publish *.tgz --provenance --access public` against `registry.npmjs.org` and hard-failed the release when `NPM_TOKEN` was missing on the canonical repo. Publishing to a public registry is not this project's release path: GitLab's `publish` job is, and it pushes the same packed tarball to the private **Dependably registry** (`$REGISTRY_URL` / `REGISTRY_PUBLISH_KEY`, a manual gate on a protected `vX.Y.Z` tag) and reads the published version back to prove it landed. Repointing the GitHub step at `$REGISTRY_URL` instead was rejected: it would give one artifact two publishers and copy a publish-scoped private-registry credential into GitHub secrets, for nothing GitLab does not already do.
  - **The signed-release property survives intact, because it never depended on the publish.** `actions/attest-build-provenance` still signs the exact `npm pack` bytes and stores the attestation on GitHub (`gh attestation verify <tarball> -R dependably/npm-check`), and the GitHub Release still carries the `.tgz`, the production SBOM and its sigstore bundle — all registry-independent, the same mechanism the NuGet pipeline uses. What goes with the publish is sigstore provenance on the *npmjs package page* (`npm audit signatures`), which only ever worked against public npmjs.
  - **All five `registry-url: https://registry.npmjs.org` inputs are removed from `actions/setup-node`, and none was load-bearing for `npm ci`.** The repo's committed `.npmrc` pins `registry=https://registry.npmjs.org/` and every `resolved` URL in `package-lock.json` is absolute; a project `.npmrc` outranks the userconfig `setup-node` writes (verified by resolving `registry` with a conflicting userconfig in place). The input's real effect is to write an `NODE_AUTH_TOKEN`-bearing `.npmrc` — publish plumbing — so it is gone with the publish, with a header comment saying why it must not be re-added.
  - The workflow's self-contradictory comment block is gone with the step it described: it claimed npm publish was "skipped (not failed)" without `NPM_TOKEN` while the step immediately below failed the release on the canonical repo. The release job's justification comment now describes what the job actually does.
  - `README.md`'s install section now states where the package comes from (the private Dependably registry, org credentials required) instead of implying a public `npm install -g` / `npx`, and points contributors without credentials at building from source.

### Fixed
- **Three facts diagnostics gained a stable `CODE:` prefix, so a consumer can finally classify them.** `NPM_MANIFEST_UNPARSEABLE:` (an unparseable `package.json`, `src/facts/workspace.js`), `NPM_TSCONFIG_UNPARSEABLE:` (an unparseable tsconfig/jsconfig), and `NPM_LOCKFILE_UNPARSEABLE:` (an unparseable lockfile, `src/facts/lockfile-graph.js`) previously read as plain prose, so sbom-reach's `diagnosticLevel` — which promotes a diagnostic to a warning only when it contains a registered `CODE:` substring — could never promote them; they silently demoted to a note, including the unparseable-manifest case, which can convert an undecidable "does this queried package collide with a first-party workspace package" check into a confident (and wrong) negative. `NO_LOCKFILE:` and `OUTPUT_DIR_SCANNED:` already followed the convention and are unchanged. `tests/unit/facts-diagnostic-codes.test.js` statically scans every `diagnostics.push(...)` call site in `src/facts/` and fails on a future uncoded one rather than letting it be silently demoted downstream, and complements that with a BEHAVIOURAL check over the strings a real `discoverWorkspace`/`discoverLockfileGraphs` run actually emits — the static scan matches the literal sink name `diagnostics.push(`, so a renamed local, an alias, or an indirect `push` is invisible to it (`src/facts/scan.js` already has a sibling string-array sink named `warnings`), while the behavioural one covers every route into the array but only the code paths a test exercises. Neither subsumes the other. The static extraction also reads EVERY argument of a variadic `push` now, not just the first. Documented in `docs/API.md` / `docs/CLI.md` ("Diagnostic codes"). checker-npm#36.
  - **No `FACTS_SCHEMA_VERSION` bump.** The version doc ties the minor to the document's *shape* — a field added, renamed, or removed — and this changes only the prose inside three existing `string[]` diagnostic entries; the field itself (`workspace.diagnostics`/`lockfile.diagnostics`, both already `string[]`) is untouched, and by the issue's own account no consumer currently depends on the old unprefixed spelling (that dependency is exactly what was impossible before this fix). The `{code, message}` restructuring the issue raises as the durable fix is deliberately out of scope for this change and would warrant its own bump when it lands.
- **Two integration assertions had been failing since the `min-release-age` rule landed in 1.10.0.** That rule defaults to `warn` and fires on any project with no committed cooldown, which every temp-dir test workspace is — a bare `package.json` + lockfile copied out of `tests/fixtures/`, no `.npmrc`. `cli-pin`'s "pinned project passes the audit pinned-versions rule" and `cli-prune`'s post-prune audit both ran `audit --strict`, where a warning is exit 1, so both went red on a rule neither test is about. The 1.10.0 commit updated the unit tests for the new rule (`audit.test.js` writes an `.npmrc`, `report.test.js` turns it off) and missed these two. Both invocations now silence the rule by id — `--rule min-release-age:off` — which keeps the assertion (the audit must still exit 0 on every other rule) instead of relaxing it; `cli-prune` already did exactly this for `pinned-versions`.
- **`npm ci` in the integration workspaces ran against whatever registry the machine defaulted to, not the one the fixtures were locked against.** The repo's own `.npmrc` pins `registry.npmjs.org` precisely because contributors commonly have a private registry as their user-level default — but `createTestWorkspace` builds each workspace under `os.tmpdir()`, outside the repo, where that pin does not apply. npm then rewrote every `resolved` host to the private default and the install died with `403 Forbidden - GET https://<private-host>/npm/glob/-/glob-8.1.0.tgz`. (That 403 is a deprecated-version policy on the private feed, not a missing path -- the same feed serves non-deprecated packages on the same URL shape; see dependably-community#700. The defect fixed here is the workspace asking that feed at all.) `runNpmCi` now passes `--registry` explicitly (overridable with `$NPM_CHECK_TEST_REGISTRY` for an internal mirror).
- **Three integration tests that genuinely need a live registry now SKIP with the reason, instead of failing as if the code were broken.** `npm-ci-migration`'s two install tests and `fix-checksums`' "real registry" test cannot run air-gapped, in a sandbox that blocks registry.npmjs.org, or behind a proxy that refuses it. A new `tests/integration/helpers/registry-availability.js` probes the exact URL the code under test will fetch — anonymously, following redirects — and carries the failure into the Jest suite title, so a skipped run reads `… — SKIPPED: registry https://registry.npmjs.org/… is unreachable (getaddrinfo ENOTFOUND …)` rather than a bare `○`. The probe is synchronous (a child `node`) because Jest has no runtime skip; it is cached per URL. No assertion was weakened and no test was deleted: the gate opens and the tests run unchanged as soon as the registry answers. `fix-checksums`' v1-refusal test needs no network and is deliberately left outside the gate.
...
### Added
- **New `imports` command and `@dependably/npm-check/facts` subpath export — the npm language-facts layer, moved here from sbom-reach.** `npm-check imports <dir>` reports what a JavaScript/TypeScript/Svelte tree imports as a JSON document: per-file import sites with the binding names each introduces and which of them the file actually references, each site resolved to the installed copy it loads, the statically resolved module graph *through* `node_modules` (symlink-aware, so pnpm's `.pnpm` layout works; `exports`/`imports` maps; `main`/`module`/`index.*`), and the dependency graph every `package-lock.json`/`pnpm-lock.yaml` under the tree records. The same is available as a library (`collectImportFacts`, `factsDocument`, and the primitives `scanSource`, `ModuleResolver`, `walkModuleGraph`, `discoverWorkspace`, `discoverLockfileGraphs`, …) with hand-written types in `src/facts/types.d.ts`.
  - **The ownership rule this implements:** the check tool owns *language facts* (what the source says — imports, bindings, resolution, the walk through `node_modules`, lockfile graphs, workspace discovery); [sbom-reach](https://github.com/dependably/sbom-reach) keeps the *verdicts* (`reachable` / `not-observed` / `unknown`, symbol intersection, confidence). The code moved verbatim from sbom-reach's `packages/analyzer-npm` at `44e9252` (`scan.ts`, `resolve.ts`, `modulegraph.ts`, `lockfile.ts`, `specifier.ts`, `workspace.ts`, its `discoverInstalledPackages` and realpath-aware relativizer, and the `bench/scan-bench.mjs` harness), converted from TypeScript to JSDoc-typed JavaScript; `index.ts`'s verdict logic and `symbols.ts` did not move.
  - **Facts are not findings — the pycheck precedent.** An import site has no severity, so the document is a *sibling document type*, not a findings envelope with a different payload: the same identity fields (`tool`, `toolVersion`, `schemaVersion`, `target`, `summary`), `findings` replaced by the facts sections, and an explicit `documentType: "imports"` discriminator — exactly the contract pycheck's `--imports` established. The findings envelope is byte-for-byte unchanged and still carries **no** `documentType`; `buildFactsEnvelope` and `DOCUMENT_TYPE_IMPORTS` in `src/schema.js` are the additions. A successful scan always exits `0`; the command never exits `1`.
  - **`unanalyzable` is load-bearing and always present.** Every first-party file that could not be read (`file` — this was a silent `continue` in the consumer), every `.svelte` file whose `<script>` extraction reported a problem (`file-partial`, its sites still reported), every `node_modules` file the walk skipped as unreadable or oversized (`node-modules-file`), and a walk cut off by its file budget (`walk`) is listed with a reason and counted in `summary`, so an absence of evidence is only ever read as a negative when the search actually ran. Reasons are path-free (`unreadable: EACCES`, never Node's message with the absolute path in it) and every path in the document is target-relative — chosen as the spelling of the target with the fewest `..` segments, so a symlinked prefix on the target (macOS `/var` → `/private/var`) with `node_modules` hoisted above it yields `../node_modules/x`, not a to-the-root chain — so the same tree gives the same bytes on any machine.
  - **Dependencies:** `fast-glob` and `ignore` join `yaml` as runtime dependencies (workspace discovery); `typescript` (`>=5.6`) is an **optional peer**, loaded lazily on the `imports` path only — the lockfile commands never load it, and the CLI exits `2` with `TYPESCRIPT_MISSING` when it is absent. The `/facts` modules are type-checked in CI (`npm run typecheck`, `tsconfig.facts.json`).
  - Parity with the sbom-reach analyzer it replaces was checked on the sbom-reach repository itself (`node_modules` installed): both report 431 `node_modules` files parsed, 73 installed package copies reached, 12 imports unresolved, 2 non-literal dynamic calls, and the same six dynamically-loading packages.
