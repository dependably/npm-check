...
### Added
- **The vendored `.dependably` conformance corpus is now replayed by the test suite.** npm-check authored most of the 28 shared cases and was the only tool in the suite that never ran them, so the corpus could not catch npm-check drifting from the contract it defines. `tests/unit/conformance-dependably.test.js` materializes each case into a temp repo and drives `loadAuditConfig()` — the real entry point rather than the primitives underneath it, so a broken loader cannot pass a case. Cases npm-check cannot replay are waived by name with a reason, cases it genuinely fails are recorded as divergences that trip the suite once fixed, and a coverage assertion fails on any case that is neither, so a newly vendored case cannot land unrun.
- **Recorded divergence:** replaying the corpus shows that a sibling tool's rule id inside `common.rules` raises `UNKNOWN_RULE` instead of being ignored. `common` is shared across the suite and npm-check already tolerates a sibling's rule id in `common.exceptions`; the `rules` map merges `common` into the tool map before rule ids are validated, so the same id errors there. Pinned by the `validation-unknown-rule-in-common-ignored` entry until the loader is fixed.
