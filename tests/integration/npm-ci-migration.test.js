...
import { runNpmCi, getNodeVersion, getNpmVersion, TEST_REGISTRY_PROBE_URL } from './helpers/npm-runner.js';
...
// The two tests below actually install the fixture — they cannot run without a
// registry that serves its locked tarballs. Probed once, here, because Jest has
// no runtime skip; the reason rides in the suite title so a skipped run never
// reads as a clean one.
const installGate = describeWithRegistry(
  'installs from the registry',
  TEST_REGISTRY_PROBE_URL
);
...
describe('Integration: npm ci Migration Validation', () => {
  ...
  installGate.describe(installGate.title, () => {
